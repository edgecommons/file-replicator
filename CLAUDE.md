...
`edgecommons` Rust library. Read the org umbrella `../CLAUDE.md` first (platform matrix, validation infra,
...
HOST / GREENGRASS / KUBERNETES via edgecommons (no platform branching).
...
`edgecommons` UNS core, released as v0.2.0 on `main`): `evt`/`cmd` now ride the library's `events()` facade +
...
surface. Only **P7 (optional core promotion of the UNS/control-surface helper into `edgecommons`)** has not
...
- `main.rs` = `EdgeCommonsBuilder::new(NAME).args(env::args_os()).build().await?` → `App::start`/`app.run`.
- Config: own subtree under `component.global`/`component.instances[]`; standard edgecommons sibling sections;
...
- **New ecosystem dep:** `aws-sdk-s3` (kinesis/sm/kms/ssm exist in edgecommons; s3 did not).
...
- **Unified namespace, edgecommons-core-owned**: `ecv1/{device}/FileReplicator/{instance}/{cmd|evt}/…`,
